...
static inline SdfTimes::const_iterator LowerBound(const SdfTimes& samples, double t)
{
    return std::lower_bound(samples.begin(), samples.end(), t);
}
static inline SdfTimeSampleMap::const_iterator LowerBound(const SdfTimeSampleMap& samples, double t)
{
    return samples.lower_bound(t);
}
    ...
    const double time, double* tLower, double* tUpper)
{
    if (samples.empty()) {
        ...
        *tLower = *tUpper = getTime(*samples.rbegin());
        ...
        auto iter = LowerBound(samples, time);
...
static bool
_GetBracketingTimeSamples(const SdfTimes &samples, double time,
